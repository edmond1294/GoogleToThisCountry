...
show_banner() {
    clear
    echo -e "${CYAN}╔═══════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║${YELLOW}★${CYAN}███${YELLOW}*${CYAN}█████████████████████████████████████████████████║${NC}"
...
setup_shortcut() {
    ...
    if [ ! -f "$LOCAL_SCRIPT" ] \vert{}\vert{} [ "$(readlink -f "$0")" != "$LOCAL_SCRIPT" ]; then
    ...
    chmod +x /usr/local/bin/gttc /usr/local/bin/sz
}
...
restart_service() {
    ...
    if command -v rc-service >/dev/null 2>&1 || [ -f /etc/alpine-release ]; then
        ...
        systemctl $action xray 2>/dev/null \vert{}\vert{} systemctl$action v2ray 2>/dev/null || true
...
show_menu() {
    ...
    echo " 0. 退出脚本"
    echo "================================================="
    echo -e " 💡 提示：后续可在命令行直接输入 ${GREEN}gttc${NC} (或 ${GREEN}sz${NC}) 呼出本菜单"
    echo "================================================="
